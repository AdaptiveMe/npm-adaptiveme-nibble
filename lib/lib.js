...
var runNibble = function (params, dir) {
  ...
  if (shell.exec(dir + platform.nibble_dir + path.sep + 'bin' + path.sep + 'adaptive-nibble-emulator ' + params).code !== 0) {
...
var deleteFolderRecursive = function(path_param) {
  if( fs.existsSync(path_param) ) {
    fs.readdirSync(path_param).forEach(function(file,index){
      var curPath = path_param + path.sep + file;
